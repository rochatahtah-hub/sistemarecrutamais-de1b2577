...
BEGIN
  ...
  SELECT pl.preco_mensal_centavos
    INTO v_preco
  FROM public.planos AS pl
  WHERE pl.id = _plano_id
    AND pl.ativo
    AND pl.publico
    AND pl.preco_mensal_centavos > 0;
  ...
  SELECT EXISTS (
    SELECT 1
    FROM public.leads_comerciais AS l
    WHERE lower(btrim(l.email)) = v_email
      AND (
        l.status = 'convertido'
        OR EXISTS (
          SELECT 1
          FROM public.pedidos_comerciais AS p
  ...
  ) RETURNING leads_comerciais.id INTO v_lead_id;
  ...
  ) RETURNING pedidos_comerciais.id INTO v_pedido_id;
