-- 1) Geração automática de slug único ao criar empresa (sufixo numérico quando repetir)
...
BEGIN
  ...
  END IF;
  _slug_limpo := btrim(regexp_replace(
    lower(coalesce(NULLIF(_slug_limpo, ''), unaccent_simples(_nome_limpo))), '[^a-z0-9]+', '-', 'g'), '-');
  IF _slug_limpo = '' THEN
...
-- 2) Alteração manual (com confirmação na tela) do identificador público
...
REVOKE ALL ON FUNCTION public.definir_slug_tenant(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.definir_slug_tenant(uuid, text) TO authenticated;