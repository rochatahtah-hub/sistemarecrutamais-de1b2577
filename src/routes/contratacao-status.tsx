...
function PaginaStatus() {
  ...
  return <main className="grid min-h-screen place-items-center bg-background px-4"><div className="max-w-lg text-center"><Icone className={`mx-auto h-12 w-12 ${aprovado ? "text-success" : falhou ? "text-destructive" : "text-gold"}`} /><h1 className="mt-5 text-3xl">{isPending ? "Consultando pagamento" : aprovado ? "Pagamento confirmado" : falhou ? "Pagamento não concluído" : "Aguardando confirmação"}</h1><p className="mt-3 text-muted-foreground">{aprovado ? "Recebemos a confirmação. As instruções para criar a empresa e o primeiro acesso serão enviadas ao responsável." : falhou ? "Você pode voltar aos planos e iniciar novamente quando desejar." : "A confirmação é feita diretamente pelo Mercado Pago. Esta página será atualizada automaticamente."}</p>{aprovado ? <Button asChild className="mt-7"><Link to="/acesso">Ir para o acesso</Link></Button> : <Button asChild className="mt-7" variant="outline"><Link to="/leads">Voltar aos planos</Link></Button>}</div></main>;
}