...
function PaginaLeads() {
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main><section className="malha-escura overflow-hidden text-sidebar-foreground"><div className="mx-auto grid min-h-[72vh] max-w-6xl content-center gap-10 px-5 py-16 lg:grid-cols-[1.1fr_.9fr] lg:py-24">
      ...
      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">{[[UsersRound,"Pessoas e vagas","Cadastros e operação reunidos em um só lugar."],[Sparkles,"Decisões claras","Indicadores e histórico para acompanhar resultados."],[LockKeyhole,"Dados separados","Cada empresa acessa somente sua própria operação."]].map(([Icone,titulo,texto]) => <article key={String(titulo)} className="border-l-2 border-sidebar-primary/60 bg-sidebar-accent/55 p-5"><Icone className="mb-3 h-5 w-5 text-sidebar-primary" /><h2 className="text-lg">{String(titulo)}</h2><p className="mt-1 text-sm leading-6 text-sidebar-foreground/65">{String(texto)}</p></article>)}</div>
    </div></section>
